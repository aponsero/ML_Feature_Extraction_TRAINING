...
def get_args(): 
  ...
  parser.add_argument('-t', '--seqtype', help='type of read 0 for viral, 1 for bacterial', default=4,
    type=int, metavar='TYPE', required=True)
...
def main():
  ...
  readtype= args.seqtype

  print("\t".join(['seq_id'] + un_kmers + ['read_type']))
  ...
  for record in SeqIO.parse(args.fasta, "fasta") :
    ...
    print("\t".join([record.id] + [str(x) for x in counts]+[str(readtype)]))    
